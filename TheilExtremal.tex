...
\author{David Salgado \footnote{Department of Methodology and Development of Statistical Production. Statistics Spain (INE)} and 
...
{\textit{pestim}} R package implements a hierarchical model to estimate the population counts of different territorial cells 
...
Examples of these functions are \textit{ratioBeta}, \textit{kummer}, \textit{Phi}, \textit{modeLambda};
	\item \textbf{Distribution-relation functions}, providing computation regarding 
...
\textit{dtriang, rtriang, ptriang, qtriang, dlambda, rlambda, rmatProb, rN0, rNt, rNtcondN0, rg, rp, alphaPrior, genAlpha, genUV}.
\item \textbf{Estimation-relation functions}, providing computation of estimates 
based upon the populations generated with the preceding functions. 
Examples of these functions are \textit{postN0, postNt, postNtcondN0}.
\end{itemize}

\section{The hierarchical model}\label{model}

In this section we will give a short description of the theoretical model 
underlaying the \textit{pestim} R package for estimation of the population 
counts at a given time moment.

A first input in the model is 
...
{\textit{pestim}} implements a function to estimate the actual population 
...
\begin{figure}\label{Tool}
\begin{center}
\includegraphics[width=1\textwidth,natwidth=1411,natheight=301]{Tool.png}
